...
if (fastify.io) {
  // Emitir um evento antes de chamar o runFlow (por exemplo, para indicar que o bot está processando)
  fastify.io.emit('bot_processing', {
    user_id: `${from}@w.msgcli.net`,
    status:  'processing'
  })
  fastify.io.to(`chat-${from}@w.msgcli.net`)
              .emit('bot_processing', {
                user_id: `${from}@w.msgcli.net`,
                status:  'processing'
              })
...
if (fastify.io) {
  // Emitir um evento após o runFlow com o conteúdo que o bot gerou
